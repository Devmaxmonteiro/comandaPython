...
while condicao != 9:
    ...
    print('1 - Cadastrar novos lanches')
    print('2 - Cadastrar bebidas')
    print('3 - Cadastro de promoção (para lanches)')
    ...
    if opcao == 1: 
        nomeLanche = str(input('Digite o nome do lanche que deseja cadastrar: '))
        precoLanche = float(input('Digite o preço do prato: '))
        manipulacaoArquivos.gravarPrato(nomeLanche, precoLanche)
        print('Parabéns!!')
        print('Você cadastrou o prato {}'.format(nomeLanche))
    ...
    if opcao == 3:
        ...
        lanchepromocao = str(input('Digite o nome do novo lanche para promoção: '))
        lanchepromocao1 = float(input('Digite o quanto estava custando o prato {}: '.format(lanchepromocao)))
        lanchepromocao2 = float(input('Digite o novo valor do prato {}, para promoção: '.format(lanchepromocao)))
        manipulacaoArquivos.gravarPromocao(lanchepromocao, precopromocao2)
        print('Promoção cadastrada com sucesso!')
        print('Nova promoção do dia: {}'.format(lanchepromocao))
        print('Que estava custando antes: R${}'.format(lanchepromocao1))
        print('Por apenas agora: R${}'.format(lanchepromocao2))

    if opcao == 4:
        print('O que deseja pedir? ')
        promocao = str(input('Deseja ver o cardápio promocional? [S/N] ')).upper()
        
        if promocao == 'S':
            arquivo = manipulacaoArquivos.lerArquivo('Promocao.txt', 'r')
            for linha in arquivo:
                codigo = linha.split(';')[0]
                lanchepromocao = linha.split(';')[1]
                precopromocao2 = float(linha.split(';')[2])
                print(codigo)
                print(lanchepromocao)
            ...
            for linha in arquivo: 
                codigo = linha.split(';')[0] 
                nomeLanche = linha.split(';')[1] 
                precoLanche = float(linha.split(';')[2]) 
                print(codigo) 
                print(nomeLanche) 
                print(precoLanche) 
        ...
        bebidasSplitted = manipulacaoArquivos.buscarBebidaPorId(pedidoBebida)
