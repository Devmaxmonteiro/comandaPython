...
def gravarLanche(nomeLanche, precoLanche):
	if os.path.isfile('lanche.txt') == True:
		file = lerArquivo('lanche.txt', 'r+')
		codigo = int(file.readlines()[-1].split(';')[0])
		file.write('{};{};{}\n'.format(codigo+1, nomeLanche, precoLanche))
		file.close()
	else:
		file = lerArquivo('pratos.txt', 'w')
		codigo = 1
		file.write('{};{};{}\n'.format(codigo, nomeLanche, precoLanche))
		file.close()
...
def gravarPromocao(lanchepromocao, precopromocao2):
	if os.path.isfile('Promocao.txt') == True:
		file = lerArquivo('Promocao.txt', 'r+')
		codigo = int(file.readlines()[-1].split(';')[0])
		file.write('{};{};{}\n'.format(codigo+1, lanchepromocao, precopromocao2))
		file.close()
	else:
		file = lerArquivo('Promocao.txt', 'w')
		codigo = 1
		file.write('{};{};{}\n'.format(codigo, lanchepromocao, precopromocao2))
		file.close()
